...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        out_loss = nn.functional.logsigmoid(torch.sum(input_vectors * output_vectors, dim=1))
        noise_loss = torch.sum(nn.functional.logsigmoid(-torch.bmm(noise_vectors, input_vectors.unsqueeze(2)).squeeze(2)), dim=1)
